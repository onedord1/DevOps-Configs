...
{{/*
Check that the user has not set both .installCRDs and .mysql-mongodb-postgresql-monitoring-cert-manager-crds.enabled or
set .installCRDs and disabled .mysql-mongodb-postgresql-monitoring-cert-manager-crds.keep.
.installCRDs is deprecated and users should use .mysql-mongodb-postgresql-monitoring-cert-manager-crds.enabled and .mysql-mongodb-postgresql-monitoring-cert-manager-crds.keep instead.
*/}}
{{- define "cert-manager.crd-check" -}}
  {{- if and (.Values.installCRDs) (.Values.crds.enabled) }}
    {{- fail "ERROR: the deprecated .installCRDs option cannot be enabled at the same time as its replacement .mysql-mongodb-postgresql-monitoring-cert-manager-crds.enabled" }}
  {{- end }}
  {{- if and (.Values.installCRDs) (not .Values.crds.keep) }}
    {{- fail "ERROR: .mysql-mongodb-postgresql-monitoring-cert-manager-crds.keep is not compatible with .installCRDs, please use .mysql-mongodb-postgresql-monitoring-cert-manager-crds.enabled and .mysql-mongodb-postgresql-monitoring-cert-manager-crds.keep instead" }}
  {{- end }}
{{- end -}}
