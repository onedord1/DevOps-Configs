# HA K8s Cluster With HA_Proxy

To set up a highly available Kubernetes cluster with three master nodes and three worker nodes without using a cloud load balancer, you can use a virtual machine to act as a load balancer for the API server. Here are the detailed steps for setting up such a cluster:

## Why Use LoadBalancer Node ?
This load balancer nodes acts as a single, stable entry point (VIP or DNS name) for clients, distributing traffic across all healthy master nodes. If one master node fails, the load balancer reroutes traffic to the remaining masters, ensuring uninterrupted access to the API server. HAProxy performs active health checks on master nodes. If a master's API server becomes unresponsive, the load balancer automatically stops routing traffic to it, ensuring only healthy nodes receive requests.

## Alternatives to HAProxy

- Cloud-native load balancers (e.g., AWS ALB/NLB, GCP Load Balancer).
- Other software LBs like nginx, keepalived, or MetalLB (for bare metal).

### Prerequisites
- 3 master nodes
- 3 worker nodes
- 1 load balancer node(HA_Proxy Node)
